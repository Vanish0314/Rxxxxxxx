...
Observable
    ...
    .Subscribe(tick =>
        Console.WriteLine($"[A][T:{Environment.CurrentManagedThreadId}] {DateTime.Now}: Tick {tick}")
    );

Console.WriteLine($"[T:{Environment.CurrentManagedThreadId}] {DateTime.Now}: Scenario A subscribed");
...
Observable
    ...
    .Subscribe(tick =>
        Console.WriteLine($"[B][T:{Environment.CurrentManagedThreadId}] {DateTime.Now}: Tick {tick}")
    );

Console.WriteLine($"[T:{Environment.CurrentManagedThreadId}] {DateTime.Now}: Scenario B subscribed");
...
Console.WriteLine($"[T:{Environment.CurrentManagedThreadId}] {DateTime.Now}: Main thread exiting (process stays alive due to non-background EventLoop thread)");
